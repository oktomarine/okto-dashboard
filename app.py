...
def load_all():
    ...
    def ws_to_df(ws):
        ...
        if not rows or len(rows) < 2:
            return pd.DataFrame()
        # Find header row: first row where first cell is a known field name or has multiple non-empty cells
        header_idx = 0
        for i, row in enumerate(rows):
            non_empty = [c for c in row if str(c).strip()]
            first = str(row[0]).strip() if row else ""
            # Skip title rows (merged/single cell) and info/instruction rows (start with ↓)
            if len(non_empty) > 1 and not first.startswith("↓") and not first.startswith("OKTO") and not first.startswith("MARKET") and not first.startswith("FREIGHT") and not first.startswith("BUNKER") and not first.startswith("FIXTURES"):
                header_idx = i
                break
        headers = rows[header_idx]
        # Skip data rows that are instruction rows (start with ↓)
...
with st.expander('🔍 Debug bilgisi'):
    st.write('fx_df shape:', fx_df.shape, '| columns:', fx_df.columns.tolist()[:5] if not fx_df.empty else 'BOŞ')
    st.write('r6_df shape:', r6_df.shape, '| columns:', r6_df.columns.tolist()[:5] if not r6_df.empty else 'BOŞ')
    st.write('b7_df shape:', b7_df.shape, '| columns:', b7_df.columns.tolist()[:5] if not b7_df.empty else 'BOŞ')
    st.write('fixtures count:', len(fixtures))
    st.write('rates dates count:', len(rates_data.get('dates',[])))
    st.write('bunker VLSFO rows:', len(bunker_data.get('VLSFO',[])))
    if not fx_df.empty:
        st.dataframe(fx_df.head(3))
    if not r6_df.empty:
        st.dataframe(r6_df.head(3))
